...
int iii;
    ...
    if( g.argc!=4 && g.argc!=5 ){
      usage("commit|create PAGENAME ?FILE? [--mimetype TEXT-FORMAT] [--technote DATETIME] [--technote-tags TAGS] [--technote-bgcolor COLOR]");
    }
    zPageName = g.argv[3];
    if( g.argc==4 ){
    ...
    if( !zETime ){
      ...
      zMETime = db_text(0, "SELECT coalesce(datetime(%Q),datetime('now'))", zETime);
      if( g.argv[2][1]=='r' ){
        event_cmd_commit(zMETime, 1, &content, zMimeType, zPageName, zTags, zClr);
        fossil_print("Created new tech note %s.\n", zMETime);
      }else{
        event_cmd_commit(zMETime, 0, &content, zMimeType, zPageName, zTags, zClr);
